...
LENGTH_STACK_MIN = 200  # analyse 200 candle data
...
TRADE_TIMEOUT = TRADE_DURATION * 60
TARGET_AMOUNT = 1000  # $1000
driver = get_driver()

def load_web_driver():
    url = f'{BASE_URL}/en/cabinet/'
    driver.get(url)
...
def calculate_stochastic(prices):
    high = max(prices)
    low = min(prices)
    stochastic = ((prices[-1] - low) / (high - low)) * 100
    return stochastic

def calculate_alligator(prices):
    jaw = sum(prices[-13:]) / 13
    teeth = sum(prices[-8:]) / 8
    lips = sum(prices[-5:]) / 5
